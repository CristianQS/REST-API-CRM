...
In this project we will use th id that autogenerate MongoDB. Mention that the **id** on MongoDB appears with **_id**.

You can look it in the next example:
...
The __id__ is autogenerated by the Database, so you don't need to write it in th JSON.


__IMPORTANT__
A photo is not required, but you have to send the photo in __Base64__
The API only accepts photos with the format .png  .jpg  or .jpeg
The __maximum size__ to upload a photo is the __1MB__
